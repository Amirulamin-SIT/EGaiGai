...
-- Events
--
CREATE DEFINER=`root`@`localhost` EVENT `order_49` ON SCHEDULE AT '2020-11-13 12:46:31' ON COMPLETION NOT PRESERVE ENABLE DO UPDATE orders SET disabled = true WHERE order_no = 49$$

CREATE DEFINER=`root`@`localhost` EVENT `orderitems_49` ON SCHEDULE AT '2020-11-13 12:46:31' ON COMPLETION NOT PRESERVE ENABLE DO UPDATE item INNER JOIN order_item ON item.iditem = order_item.item_iditem SET item.quantity = (item.quantity - order_item.quantity) WHERE order_item.orders_order_no = 49$$

CREATE DEFINER=`root`@`localhost` EVENT `order_50` ON SCHEDULE AT '2020-11-13 12:56:24' ON COMPLETION NOT PRESERVE ENABLE DO UPDATE orders SET disabled = true WHERE order_no = 50$$

CREATE DEFINER=`root`@`localhost` EVENT `orderitems_50` ON SCHEDULE AT '2020-11-13 12:56:24' ON COMPLETION NOT PRESERVE ENABLE DO UPDATE item INNER JOIN order_item ON item.iditem = order_item.item_iditem SET item.quantity = (item.quantity - order_item.quantity) WHERE order_item.orders_order_no = 50$$
